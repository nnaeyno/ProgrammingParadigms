...
// check to see if the word is nonTerminal or Terminal 
bool isTerminal(string testTerminal){
  //<...>
  if(testTerminal[0] == '<' && testTerminal[testTerminal.length()-1] == '>')
  return false;
  return true;
}

void generateVersion(string word, map<string, Definition>& grammar){
  if(grammar.find(word) == grammar.end())
    exit(EXIT_FAILURE);
  Definition defi = grammar[word]; 
  Production prod = defi.getRandomProduction();
  for (Production::iterator curr = prod.begin(); curr != prod.end(); ++curr) {
    if(!isTerminal(*curr)){
      generateVersion(*curr, grammar);
    } else {
      cout << *curr << " ";
    }
  }
...
//main function for generating all n number of versions
void printText(int versions, map<string, Definition>& grammar){
  string start = "<start>";
  for(int i = 1; i <= versions; i++){
    cout << "Version #"  << i  << " : --------------------------" << endl;
    generateVersion(start, grammar);
    cout << endl;
  }
