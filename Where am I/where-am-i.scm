...
;;
(define (notVisited elem lst)
  (cond
    ((null? lst) #t)
    ((equal? elem (car lst)) #f)
    (notVisited elem (cdr lst))
  )
)


(define (intersect-helper elem lst)
  (cond
    ((null? lst) '())
    ( 
      (map (lambda (x) (intersect elem x))
      lst)
    )
  )
)
(define (func lst)
 (cond
    ((null? lst) '())
    ((append (intersect-helper (car lst) (cdr lst))  (func (cdr lst)))
    )

  )
)
(define (intersection-points lst)
  (apply append (func lst))

)

(define (dist-wrapper elem x)
  (if (equal? elem x) 1
      (dist x elem)
  )
)
(define (distance-product elem lst)
  (apply * (map (lambda (x) (dist-wrapper x elem)) lst))
)
(define (rate-wrapper x lst)
...
(define (filter lst) 
  (append (map (lambda (x) (cadr x)) lst))
)
(define (clumped-points lst)
  (filter (prefix-of-list (sort-points (rate-points lst)) (quotient (length lst) 2)))
)
(define (average lst which)
  (cond
    ((null? lst) 0)
    ((equal? which 1) (/ (apply + (map (lambda (x) (car x)) lst)) (length lst)))
    ((/ (apply + (map (lambda (x) (cadr x)) lst)) (length lst)))
  )
)
(define (average-point lst)
  (let 
        ((xAverage (average lst 1)) (yAverage (average lst 2)))
        (list (distance-product (list xAverage yAverage) lst) (list xAverage yAverage))
  )
)
...
(define (where-am-i dst stars)
  (sort-points (map (lambda (x) (best-estimate x)) (all-guesses dst stars)
               )
  )
)
;;; --------------------------------------------------------------------------------------
