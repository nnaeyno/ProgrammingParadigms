 ...
 */
void finishLine(Bank* bank, int workNum){
  for(int i = 0; i < numWorkers; i++){ //num??
      if(i == workNum) continue;
      sem_post(&(bank->dayStarter[i]));
    }
    bank-> waitingFor = numWorkers;
    sem_post(&bank->lockWait);
}
int Report_DoReport(Bank *bank, int workerNum)
{
  sem_wait(&bank->lockWait);
  bank->waitingFor--;
  if(bank->waitingFor==0){
   ...
   */
    ...
    sem_post(&bank->lockWait);
    sem_wait(&(bank->dayStarter[workerNum]));
    return 0;
  }
  return 0;
}
